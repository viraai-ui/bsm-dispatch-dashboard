...
export async function POST() {
  ...
  return apiOk({ reset: true, mediaFilesFound: files.length, mediaFilesDeleted: deleteResults.deleted, mediaDeleteErrors: deleteResults.errors })
}

function collectMediaFiles(store: MediaProofStore) {
  const files: MediaUpload[] = []
  for (const record of Object.values(store.records || {})) {
    for (const unit of Object.values(record.units || {})) files.push(...(unit.photos || []), ...(unit.videos || []))
  }
  return files
}

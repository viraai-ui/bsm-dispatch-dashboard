...
function RecordModal({ order, media, loadingMedia, status, warrantyDate, shipment, onClose }: { order: Order; media?: MediaProofRecord; loadingMedia?: MediaProofRecord; status?: OrderStatusProjection; warrantyDate?: string; shipment?: ShipmentRecord; onClose: () => void }) {
  ...
  return <div className="modal-backdrop" role="dialog" aria-modal="true"><section className="order-modal card"><div className="modal-head"><div><h1>{order.salesOrderNumber}</h1><p className="muted">{order.customerName}</p></div><button className="drawer-close" onClick={onClose}>×</button></div>
    ...
    {shipment && <section className="modal-section database-shipment-section"><h2>Dispatch Details</h2><div className="grid two details-grid"><Info k="Shipment Type" v={shipment.shipmentType === 'transporter' ? 'Transporter' : 'Direct'} /><Info k="Transporter Name" v={shipment.transporterName || '—'} /><Info k="Transporter Number" v={shipment.transporterPhone || '—'} /><Info k="Loading Video" v={<VideoButtons files={loadingVideos} label="View Loading Video" />} />{shipment.shipmentType !== 'transporter' && <><Info k="Vehicle Number" v={shipment.vehicleNumber || '—'} /><Info k="Driver Mobile" v={shipment.driverPhone || '—'} /><Info k="Expected Delivery" v={formatDisplayDate(shipment.expectedDelivery)} /></>}{shipment.lrCopy?.url && <Info k="Builty/LR Copy" v={<a href={shipment.lrCopy.url} target="_blank">{shipment.lrCopy.name || 'View attachment'}</a>} />}{shipment.notes && <Info k="Notes" v={shipment.notes} />}</div></section>}
  </section></div>
}

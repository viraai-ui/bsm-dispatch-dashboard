...
export function OrdersClient({ orders, live = false }: { orders: Order[]; live?: boolean }) {
  ...
  useEffect(() => {
    ...
    void fetch('/api/auth/me', { cache: 'no-store' }).then((r) => r.json()).then((json) => setIsAdmin(json?.user?.role === 'Admin')).catch(() => setIsAdmin(false))
  ...
  return <>
    ...
    {active && <OrderModal order={active} stage={activeStage} workflow={activeWorkflow} paymentStatus={paymentStatus(active)} isAdmin={isAdmin} onClose={() => setActive(null)} onSynced={(synced) => { setActive(synced); setRows((items) => { const next = items.map((item) => item.id === synced.id ? synced : item); cacheOrders(next); return next }) }} onCancelled={(cancelled) => { setRows((items) => { const next = items.filter((item) => item.id !== cancelled.id); cacheOrders(next); return next }); setActive(null); setNotice(`${cancelled.salesOrderNumber} removed from dashboard`) }} />}
  </>
}

function OrderModal({ order, stage, workflow, paymentStatus, isAdmin, onClose, onSynced, onCancelled }: { order: Order; stage: OrderStage; workflow: OrderWorkflow | null; paymentStatus?: ProjectedPaymentStatus; isAdmin: boolean; onClose: () => void; onSynced: (order: Order) => void; onCancelled: (order: Order) => void }) {
  ...
  const cancelOrder = async () => {
    ...
    try {
      const response = await fetch(`/api/orders/${encodeURIComponent(order.id)}/cancel`, { method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ salesOrderNumber: order.salesOrderNumber, reason: 'Cancelled from dashboard' }) })
  ...
  const syncThisOrder = async () => {
    ...
    setSyncingOrder(true); setMessage('')
    try {
      const response = await fetch(`/api/orders/${encodeURIComponent(order.id)}/sync`, { method: 'POST', cache: 'no-store' })
      const json = await response.json().catch(() => ({}))
      if (!response.ok || !json.ok) throw new Error(json.error || 'Could not sync this order')
      const synced = json.data.order as Order; const diff = json.data.diff
      setMachines(synced.machines); setSelected(new Set(synced.machines.filter((m) => m.selectedForBatch && !isLockedMachine(m)).map((m) => m.id))); onSynced(synced)
      if (!json.data.changed) setMessage('Order is already up to date')
      else { const parts = [`${order.salesOrderNumber} synced`]; if (diff.added.length) parts.push(`${diff.added.length} item${diff.added.length === 1 ? '' : 's'} added`); if (diff.removed.length) parts.push(`${diff.removed.length} item${diff.removed.length === 1 ? '' : 's'} removed`); if (diff.updated.length) parts.push(`${diff.updated.length} updated`); setMessage(parts.join(' • ')) }
    } catch (error) { setMessage(error instanceof Error ? error.message : 'Could not sync this order. Current data was kept.') }
    finally { setSyncingOrder(false) }
  }

  return <div className="modal-backdrop" role="dialog" aria-modal="true"><section className="order-modal card"><div className="modal-head"><div><h1>{order.salesOrderNumber}</h1><div className="order-status-strip"><Badge tone={stageTone(modalStage)}>{stageLabel(modalStage)}</Badge><PaymentStatusChip status={paymentStatus} /></div></div><div className="modal-head-actions"><button type="button" className={`btn light order-sync-btn${syncingOrder ? ' syncing' : ''}`} aria-label="Sync this order from Zoho" title="Sync this order from Zoho" disabled={syncingOrder} onClick={() => void syncThisOrder()}><span aria-hidden="true">⟳</span><b>{syncingOrder ? 'Syncing…' : 'Sync'}</b></button><div className="order-menu-wrap"><button className="drawer-close menu-dot-btn" aria-label="Order actions" onClick={() => setMenuOpen((open) => !open)}>⋯</button>{menuOpen && <div className="order-menu"><button type="button" onClick={undoOrder} disabled={undoing}>{undoing ? 'Undoing…' : 'Undo Order'}</button>{isAdmin && <button type="button" className="cancel-order-menu-item" onClick={() => { setMenuOpen(false); setCancelConfirm(true) }}><span aria-hidden="true">⊘</span> Cancel Order</button>}</div>}</div><button className="drawer-close" onClick={onClose}>×</button></div></div>
    {message && <div className={message.toLowerCase().includes('success') || message.includes(' synced') || message === 'Order is already up to date' || message.startsWith('Serial saved;') || message.startsWith('Barcode PDF generated') ? 'form-success process-success-tape' : 'form-error'}>{message}</div>}
    ...
    {cancelConfirm && <div className="modal-backdrop nested cancel-confirm-backdrop" role="dialog" aria-modal="true" aria-labelledby="cancel-order-title"><section className="card cancel-order-confirm"><h2 id="cancel-order-title">Cancel {order.salesOrderNumber}?</h2><p>This removes it from the active dashboard and operational queues. It does not cancel the order in Zoho. Workflow, serial, media, shipment, payment, and database history are preserved.</p><div className="modal-actions"><button className="btn light" disabled={cancelling} onClick={() => setCancelConfirm(false)}>Keep Order</button><button className="btn red" disabled={cancelling} onClick={() => void cancelOrder()}>{cancelling ? 'Removing…' : 'Cancel Order'}</button></div></section></div>}

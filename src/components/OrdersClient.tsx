...
export function OrdersClient({ orders, live = false }: { orders: Order[]; live?: boolean }) {
  ...
  return <>
    ...
    {active && <OrderModal order={active} stage={activeStage} workflow={activeWorkflow} paymentStatus={paymentStatus(active)} isAdmin={isAdmin} onClose={() => setActive(null)} onCancelled={(cancelled) => { setRows((items) => { const next = items.filter((item) => item.id !== cancelled.id); cacheOrders(next); return next }); setActive(null); setNotice(`${cancelled.salesOrderNumber} removed from dashboard`) }} />}
  </>
}

function OrderModal({ order, stage, workflow, paymentStatus, isAdmin, onClose, onCancelled }: { order: Order; stage: OrderStage; workflow: OrderWorkflow | null; paymentStatus?: ProjectedPaymentStatus; isAdmin: boolean; onClose: () => void; onCancelled: (order: Order) => void }) {

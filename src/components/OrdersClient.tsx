...
export function OrdersClient({ orders, live = false }: { orders: Order[]; live?: boolean }) {
  ...
  useEffect(() => {
    ...
    void fetch('/api/auth/me', { cache: 'no-store' }).then((r) => r.json()).then((json) => setCanCancel(['Admin', 'Operations'].includes(json?.user?.role))).catch(() => setCanCancel(false))

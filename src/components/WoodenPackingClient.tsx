...
export function WoodenPackingClient({ initialQueue = { items: [] } }: { initialQueue?: WoodenQueue }) {
  ...
  return <>
    <header className="top compact-top wooden-mobile-head"><div><h1 className="h1">Wooden Packing</h1></div><button className="btn light wooden-sync-btn" aria-label="Sync" title="Sync" onClick={() => syncZoho(true)} disabled={syncing}>{syncing ? '↻' : '⟳'}</button></header>

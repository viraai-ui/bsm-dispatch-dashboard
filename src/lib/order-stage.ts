...
export type OrderStage = 'open' | 'processed' | 'packed' | 'media_uploaded' | 'dispatched' | 'closed'
export type DispatchStore = { dispatched: Record<string, { dispatchedAt: string; order: Order }> }
export type CompletedStore = { completed: Record<string, { completedAt: string; order: Order }> }

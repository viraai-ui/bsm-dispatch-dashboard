...
export type StoredPushSubscription = WebPushSubscription & { userId: string; role: 'Accounts'; createdAt: string; updatedAt: string }
...
export async function savePaymentPushSubscription(userId: string, subscription: WebPushSubscription) {
  const now = new Date().toISOString()
  await updateStore((items) => [{ ...subscription, userId, role: 'Accounts', createdAt: items.find((item) => item.endpoint === subscription.endpoint)?.createdAt || now, updatedAt: now }, ...items.filter((item) => item.endpoint !== subscription.endpoint)])
}
...
export async function notifyAccountsOfNewPayment(payment: { id: string; customerName: string; salesOrderNumber: string; paymentAmount?: number }) {
  ...
  const accounts = (data.subscriptions || []).filter((item) => item.role === 'Accounts')
  const dead = new Set<string>()
  let sent = 0
  const amount = payment.paymentAmount == null ? '' : ` • ₹${payment.paymentAmount.toLocaleString('en-IN')}`
  await Promise.allSettled(accounts.map(async (subscription) => {
    try {

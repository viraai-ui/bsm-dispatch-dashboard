...
test('payment amount and mode are persisted and validated', async () => {
  ...
  assert.match(route, /createPayment\(\{ customerName: linked \? authoritativeOrder!\.customerName : customerName,[\s\S]*paymentAmount, paymentMode,/)
})
...
test('payment form follows required order and list exposes responsive details', async () => {
  ...
  const labels = ['Sales Order (optional)', 'Customer Name', 'Payment Amount', 'Payment Mode']
  let previous = -1
  ...
  assert.match(client, /required maxLength=\{120\} readOnly=\{Boolean\(selectedOrder\)\} value=\{customerName\}/)

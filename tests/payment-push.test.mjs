...
test('one to ten proofs are required while core fields remain server validated', async () => {
  ...
  assert.match(route, /Boolean\(salesOrderId\) !== Boolean\(salesOrderNumber\)/)
  assert.match(route, /if \(!customerName\)/)
  assert.match(route, /linked \? await validatePaymentOrder\(salesOrderId, salesOrderNumber, customerName\) : null/)
  assert.match(route, /!linked && !manualScope/)

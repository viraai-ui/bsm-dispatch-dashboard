...
test('mobile payments use deliberate responsive cards with complete functionality', async () => {
  ...
  assert.match(client, /payment-mobile-proof[\s\S]*payment-mobile-proof-icon[\s\S]*<span>View proof<\/span>/)
  ...
  assert.match(css, /\.payments-page \.payment-mobile-proof\{[^}]*height:35px;min-height:35px[^}]*font-size:12px[^}]*text-decoration:none[^}]*white-space:nowrap/)
  assert.match(css, /\.payments-page \.payment-mobile-proof::before\{[^}]*inset:-5px 0/)
...
test('internal payments header and mobile cards stay rail-free while public payments remain isolated', async () => {
  ...
  assert.doesNotMatch(publicUi, /payment-mobile-proof-icon|View proof/)

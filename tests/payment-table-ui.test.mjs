...
test('payment table has requested column order and enabled status controls for both payment roles', async () => {
  ...
  assert.match(client, /<th>Date<\/th><th>Sales Order<\/th><th>Customer Name<\/th><th>Mode<\/th><th>Amount<\/th><th>Proofs<\/th><th>Remarks<\/th>/)
  assert.match(client, />View Proof\{payment\.attachments\.length > 1/)
  ...
  assert.match(client, /setViewer\(payment\.attachments!\.map/)
  assert.doesNotMatch(client, /target="_blank"|window\.open\(/)
...
test('mobile payments use deliberate responsive cards with complete functionality', async () => {
  ...
  assert.match(client, /payment-mobile-proof[\s\S]*payment-mobile-proof-icon[\s\S]*<span>Screenshot<\/span>/)
  assert.doesNotMatch(client, /<span>View proof<\/span>/)
  assert.match(client, /No proof/)
  ...
  assert.match(client, /payment-mobile-status-control \$\{payment\.status === 'Payment Received' \? 'received' : 'pending'\}/)
  assert.match(client, /<option value="Pending">Pending<\/option><option value="Payment Received">Received<\/option>/)
  assert.match(css, /\.payments-page \.payment-mobile-footer\{[^}]*grid-template-columns:118px 118px[^}]*align-items:center/)
  assert.match(css, /\.payments-page \.payment-mobile-proof\{[^}]*width:118px;min-width:118px;max-width:118px;height:35px;min-height:35px[^}]*border-radius:10px[^}]*font-size:12px;font-weight:600/)
  assert.match(css, /\.payments-page \.payment-mobile-status-control\{[^}]*width:118px;min-width:118px;max-width:118px;height:35px;min-height:35px[^}]*border-radius:10px/)
  assert.match(css, /\.payments-page \.payment-mobile-status-control\.received\{[^}]*background:#edf9f0[^}]*color:#287044/)
  assert.match(css, /\.payments-page \.payment-mobile-status\{[^}]*height:35px!important[^}]*font-size:12px!important;font-weight:600[^}]*-webkit-appearance:none;appearance:none/)
  assert.match(css, /payment-mobile-status-control::before\{[^}]*inset:-5px 0/)
  assert.match(css, /\.payments-page \.payment-mobile-proof:focus-visible\{[^}]*outline:2px solid/)
  assert.match(css, /\.payments-page \.payment-mobile-proof-icon\{[^}]*width:15px;height:15px/)
...
test('internal payments header and mobile cards stay rail-free while public payments remain isolated', async () => {
  ...
  assert.doesNotMatch(publicUi, /payment-mobile-proof-icon|View proof|<span>Screenshot<\/span>|payment-mobile-status-control/)

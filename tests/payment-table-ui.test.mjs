...
test('payment table has requested column order, compact proof link, and role-specific status controls', async () => {
  ...
  assert.match(client, /<select className=\{`payment-status-select[\s\S]*disabled=\{!isAccounts[\s\S]*<option value="Pending">Pending<\/option><option value="Payment Received">Payment Received<\/option>/)

...
test('mobile public database has premium app shell and compact cards', () => {
  ...
  assert.match(component, />View <Icon name="chevron"/)
...
test('detail is lazy, accessible and attachment actions navigate safely', () => {
  ...
  assert.match(component, />View<\/a>/)
  assert.doesNotMatch(component, /View Builty \/ LR|View loading video|View packing video/)
...
test('desktop uses a premium operational table and public client stays server-paginated/read-only', () => {
  ...
  assert.match(component, /<th>Warranty Status<\/th><th>Warranty Valid Till<\/th>/)
  assert.doesNotMatch(component, /Status & media|pdb-table-statuses|View record/)
  assert.match(component, /Warranty Valid/)
  assert.match(component, /Warranty Void/)
  assert.match(component, /valid \? 'green' : 'red'/)
  assert.match(component, /aria-label=\{`View \$\{row\.salesOrderNumber\}`\}/)
  assert.match(css, /\.pdb-status\.red\{background:#fdebec;color:#a12632\}/)

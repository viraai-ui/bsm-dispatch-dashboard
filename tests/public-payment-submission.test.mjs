...
test('public dashboard has list, read-only statuses, add form, polling and responsive cards', async () => {
  ...
  assert.match(client, /Date.*Sales Order.*Customer Name.*Mode.*Amount.*Proofs.*Remarks.*Status/s)
...
test('public list exposes presentation fields only and proof lookup never accepts a caller key', async () => {
  ...
  assert.doesNotMatch(proof, /get\('key'\)/)
  assert.match(proof, /searchParams\.get\('index'\)/)
  assert.match(proof, /paymentAttachments\(payment\)\[Number\(indexText\)\]/)
